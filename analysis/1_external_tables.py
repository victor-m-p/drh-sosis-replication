"""
Format 1_external.Rmd's saved CSVs (data/model/external/results) into LaTeX tables
(data/model/external/tables). diagnostics_* are excluded — convergence QA, not a paper table.
"""
...
IN = Path("../data/model/external/results")
OUT = Path("../data/model/external/tables")
OUT.mkdir(parents=True, exist_ok=True)

# short, unique-across-pipelines tag for \label{} keys (fixed_effects_baseline etc. would
# otherwise collide with the same table in 2_internal_tables.py and friends); not shown in
