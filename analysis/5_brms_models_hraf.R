...
entries <- read_csv(here("data", "preprocessed", "entries_clean.csv"))
...
file_paths <- list.files(path = directory_path, pattern = "\\.csv$", full.names = TRUE, ignore.case = TRUE)
stopifnot(length(file_paths) > 0)

# output
out_dir <- here("data", "mdl_output_hraf")
...
for (file_path in file_paths) {
  ...
  print(unique_entries_count) # n=356 (v1)
  ...
  unique_entries_count_filtered <- data %>%
    summarise(unique_entries = n_distinct(entry_id))
  print(unique_entries_count_filtered) # n=346 (v1)
